...
impl<D> InnerBackend<D> {
    pub fn new() -> Result<Self, InitError> {
        ...
        // illumos uses native event ports; see event_port.
        #[cfg(target_os = "illumos")]
        let poll_fd = super::event_port::create().map_err(InitError::Io)?;
    ...
    pub fn dispatch_all_clients(&self, data: &mut D) -> std::io::Result<usize> {
        ...
        let mut dispatched = 0;
        loop {
            let ready = super::event_port::wait_nonblocking(&poll_fd.as_fd(), 32)?;
            ...
            for tag in ready {
                let id = InnerClientId::from_u64(tag);
                // remove the cb while we call it, to gracefully handle reentrancy
                //
                // Event-port associations are one-shot: delivery consumed this
                // client's registration. Re-arm it afterwards, unless dispatch
                // hit a hard error (that path already dissociated and killed
                // the client; re-arming would make a dead client's fd fire
                // forever). A WouldBlock result is a spurious wakeup and must
                // still re-arm.
                let rearm = match self.dispatch_events_for(data, id.clone()) {
                    Ok(count) => {
                        dispatched += count;
                        true
                    }
                ...
                if rearm {
                    ...
                    if let Ok(client) = state.clients.get_client(id) {
                        let _ = super::event_port::add(&state.poll_fd, client, tag);
                    }
                }
            }
            let cleanup = self.state.lock().unwrap().cleanup();
                ...
                if let Ok(client) = state.clients.get_client_mut(client_id.clone()) {
                    let (message, object) = match client.next_request() {
                        ...
                        Err(e) => {
                            ...
                            #[cfg(target_os = "illumos")]
                            {
                                super::event_port::delete(&state.poll_fd, client)?;
                            }
                            return Err(e);
                        }
                    };
