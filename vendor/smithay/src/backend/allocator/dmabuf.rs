...
// WayRay illumos patch: rustix gates `ioctl::opcode` off illumos, so spell out
// the same `_IOW('b', 0, dma_buf_sync)` encoding. dma-buf is Linux-only and
// this ioctl is never issued on illumos; the constant just needs to compile.
#[cfg(target_os = "illumos")]
const DMA_BUF_SYNC: rustix::ioctl::Opcode = ((1u32 << 30)
    | ((std::mem::size_of::<dma_buf_sync>() as u32) << 16)
    | ((b'b' as u32) << 8)) as rustix::ioctl::Opcode;
