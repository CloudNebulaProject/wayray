...
unsafe fn place_sigbus_handler() {
    let _ = OLD_SIGBUS_HANDLER.get_or_init(|| {
        ...
        unsafe {
            ...
            // WayRay patch: go through a function pointer so the cast to
            // sighandler_t is pointer->integer, not fn-item->integer (the
            // latter trips the `function_casts_as_integer` lint, which fails
            // builds of this path dependency under RUSTFLAGS=-D warnings).
            let handler: extern "C" fn(libc::c_int, *mut libc::siginfo_t, *mut libc::c_void) =
                sigbus_handler;
            action.sa_sigaction = handler as usize as _;
...
// WayRay illumos patch: illumos libc exposes `si_addr` as a method.
